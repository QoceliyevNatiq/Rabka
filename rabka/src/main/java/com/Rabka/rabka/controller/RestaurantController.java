...
public class RestaurantController {
    ...
    @PostMapping
    public ResponseEntity<RestaurantResponseDto> create(@RequestBody @Valid RestaurantCreateDto dto) {
    ...
    @PatchMapping("/update")
    public ResponseEntity<RestaurantResponseDto> update(@RequestBody @Valid RestaurantUpdateDto dto) {
        log.debug("Rest request to update restaurant | id: {}", dto.id());
        RestaurantResponseDto response = restaurantService.updateRestaurant(dto);
        log.info("Rest request to update restaurant | id: {}", response.id());
        return ResponseEntity.ok().body(response);
    }

    @PatchMapping("/status/{id}")
    public ResponseEntity<RestaurantResponseDto> updateStatus(@PathVariable Long id, @RequestParam RestaurantStatus status) {
        log.debug("Rest request to update restaurant status | id: {}, status: {}", id, status);
        RestaurantResponseDto response = restaurantService.updateRestaurantStatus(id, status);
        log.info("Rest request to update restaurant status | id: {}", response.id());
        return ResponseEntity.ok().body(response);
    }

    @GetMapping("/active")
    public ResponseEntity<Page<RestaurantResponseDto>> getAllActive(@PageableDefault(size = 20, sort = "id", direction = Sort.Direction.DESC) Pageable pageable) {
        log.debug("Rest request to get all active restaurants");
        Page<RestaurantResponseDto> response = restaurantService.getAllRestaurantsIsActive(pageable);
        log.info("Rest request to get all active restaurants | totalElements: {}", response.getTotalElements());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/open")
    public ResponseEntity<Page<RestaurantResponseDto>> getOpenRestaurants(@RequestParam Time now, @PageableDefault(size = 20, sort = "id", direction = Sort.Direction.DESC) Pageable pageable) {
